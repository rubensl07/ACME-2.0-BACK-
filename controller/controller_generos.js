        ...
        if(String(contentType).toLowerCase()== 'application/json'){
            ...
            if (dadosGenero.nome == ''|| dadosGenero.nome == undefined|| dadosGenero.nome == null||dadosGenero.nome.length > 30) {
               return message.ERROR_REQUIRED_FIELDS //400
                    ...
                    if(novoGenero) {
                        novoGeneroJSON.genero = dadosGenero
                        novoGeneroJSON.status = message.SUCCESS_ACCEPTED_ITEM.status
                        novoGeneroJSON.status_code = message.SUCCESS_ACCEPTED_ITEM.status_code
                        novoGeneroJSON.message = message.SUCCESS_ACCEPTED_ITEM.message
...
const setExcluirGenero = async function (id) {
    let excluirGeneroJSON={}
    let generoExcluido = await DAO.deleteGenero(id)
    if(generoExcluido){
        excluirGeneroJSON.status = message.SUCCESS_ACCEPTED_ITEM.status
        excluirGeneroJSON.status_code = message.SUCCESS_ACCEPTED_ITEM.status_code
        excluirGeneroJSON.message = message.SUCCESS_ACCEPTED_ITEM.message
        return excluirGeneroJSON //202
    } else {
        return message.ERROR_NOT_FOUND //404
    }
}
...
const setInserirGeneroFilme = async function (dados,contentType) {
    ...
    if (dados.idFilme == ''|| dados.idFilme == undefined|| dados.idFilme == null||isNaN(dados.idFilme)||
        dados.idGenero == ''|| dados.idGenero == undefined|| dados.idGenero == null||isNaN(dados.idGenero)
){
       return message.ERROR_REQUIRED_FIELDS //400
    } else {
...
const setExcluirGeneroFilme = async function (dadosBody,contentType) {
    if (String(contentType).toLowerCase() == 'application/json') {
        ...
        if(generoExcluido){
            excluirGeneroFilmeJSON.status = message.SUCCESS_ACCEPTED_ITEM.status
            excluirGeneroFilmeJSON.status_code = message.SUCCESS_ACCEPTED_ITEM.status_code
            excluirGeneroFilmeJSON.message = message.SUCCESS_ACCEPTED_ITEM.message
